// server.js — MoonHub key backend with Linkvertise Anti-Bypass (Node >=20)
...
async function verifyLinkvertiseHash(hash) {
  if (!LINKVERTISE_AUTH_TOKEN) { console.warn("LV token missing"); return { ok:false, detail:"no_token" }; }
  if (!hash || hash.length < 32) { console.warn("LV hash short"); return { ok:false, detail:"no_hash" }; }
  ...
  try {
    ...
    try { const j = JSON.parse(t); if (j === true || j?.valid === true) return { ok:true }; } catch {}
...
app.get("/lvreturn", async (req,res) => {
  ...
  if (!v.ok) {
    ...
    return res.status(403).type("text/plain")
      .send("invalid hash from Linkvertise. Ensure Anti-Bypass is enabled, Target URL = /lvreturn, and do not reload.\nDetails: " + (v.detail||"") + (v.body?("\nBody: "+v.body):""));
  }
