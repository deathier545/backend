...
// - Linkvertise anti-bypass verify (hardcoded token)
// - Invisible Cloudflare Turnstile human check with strict server validation
// - IP/UA‑bound signed flow cookie
// - Signed step cookies (mh_lv_done, mh_human)
// - Single‑use key retrieval; `/get` ignores query uid and reads from signed flow
// - No cooldown, no rate limiting, no auto‑blocking
//
// Required env vars:
//   SECRET                HMAC secret for keys and cookie signatures
//   LINKVERTISE_URL       Your Linkvertise campaign URL
//   TURNSTILE_SITEKEY     Cloudflare Turnstile site key
//   TURNSTILE_SECRET      Cloudflare Turnstile secret key
//
// Optional env vars:
//   GATE_TTL_MS           ms window to complete LV + human + claim (default 600000)
//   TOKEN_TTL_SEC         /verify token lifetime in seconds (default 86400)
//   GRACE_PREV_DAY        "true"/"false" accept yesterday's key (default "true")
//   COOKIE_SECURE         "true"/"false" set Secure on cookies (default "true")
...
// Restrictive CORS for browsers (Roblox executors ignore CORS)
...
function validHash(h) {
  ...
  if (h.length < 16 || h.length > 256) return false;
  return /^[A-Za-z0-9._\-]+$/.test(h); // allow base64url-ish
}
async function verifyLinkvertiseHash(hash) {
  if (!LINKVERTISE_AUTH_TOKEN) return { ok: false, detail: "no_token" };
  if (!validHash(hash))         return { ok: false, detail: "bad_hash" };
  ...
  try {
    ...
    if (r.ok && (t.toUpperCase() === "TRUE" || t === "true")) return { ok: true };
...
// /get requires valid signed flow + signed step cookies; single‑use. Ignores query uid.
app.get("/get", (req,res) => {
  const flow = String(req.cookies.mh_flow||"");
  const pf = flow.split(".");
  if (pf.length !== 6) return res.status(403).json({ ok:false, msg:"forbidden" });
  const [uid, ts, nonce, ipH, uaH, sig] = pf;
  const body = `${uid}.${ts}.${nonce}.${ipH}.${uaH}`;
  if (sig !== hmacHex(SECRET, body)) return res.status(403).json({ ok:false, msg:"forbidden" });
  const age = Date.now() - Number(ts);
  if (!(age >= 0 && age <= GATE_TTL_MS)) return res.status(403).json({ ok:false, msg:"expired" });
  const nowIpH = shaFrag(ipCidrKey(ipOf(req)));
  const nowUaH = shaFrag(String(req.get("user-agent")||"").slice(0,64));
  if (ipH !== nowIpH || uaH !== nowUaH) return res.status(403).json({ ok:false, msg:"moved" });

  const lv = String(req.cookies.mh_lv_done||"").split(".");         // uid.ts.sig
  const hu = String(req.cookies.mh_human||"").split(".");           // uid.ts.sigH
  if (lv.length!==3 || hu.length!==3) return res.status(403).json({ ok:false, msg:"gate first" });
  if (lv[0]!==uid || hu[0]!==uid || lv[1]!==ts || hu[1]!==ts) return res.status(403).json({ ok:false, msg:"gate first" });
  if (lv[2]!==hmacHex(SECRET, `${uid}.${ts}`))       return res.status(403).json({ ok:false, msg:"gate first" });
  if (hu[2]!==hmacHex(SECRET, `${uid}.${ts}.H`))     return res.status(403).json({ ok:false, msg:"gate first" });
...
app.post("/verifyToken", (req,res)=>{
  ...
  if (!v.ok || String(v.payload.uid)!==String(uid)) return res.json({ ok:false, msg:"invalid" });
...
app.get("/lvreturn", async (req,res)=>{
  const flow = String(req.cookies.mh_flow||"");
  const p = flow.split(".");
  if (p.length !== 6) return res.status(403).send("forbidden");
  const [uid, ts, nonce, ipH, uaH, sig] = p;
  const body = `${uid}.${ts}.${nonce}.${ipH}.${uaH}`;
...
<body>
  ...
  // mark LV done — uid.ts.sig
  const mark = `${uid}.${ts}`;
  res.cookie("mh_lv_done", `${mark}.${hmacHex(SECRET, mark)}`, { maxAge:GATE_TTL_MS, httpOnly:true, sameSite:"Lax", secure:COOKIE_SECURE });

  const expectedCData = crypto.createHash("sha256").update(hmacHex(SECRET, body)).digest("hex").slice(0,16);
...
app.post("/human", async (req,res)=>{
  ...
  // mark Human done — uid.ts.sigH
  const mark = `${uid}.${ts}`;
  res.cookie("mh_human", `${mark}.${hmacHex(SECRET, `${mark}.H`)}`, { maxAge:GATE_TTL_MS, httpOnly:true, sameSite:"Lax", secure:COOKIE_SECURE });
  res.status(200).json({ ok:true });
});

// Step 3 — key page (requires valid flow + both signed step cookies)
app.get("/keygate", (req,res)=>{
  ...
  const lv = String(req.cookies.mh_lv_done||"").split(".");
  const hu = String(req.cookies.mh_human||"").split(".");
  if (lv.length!==3 || hu.length!==3) return res.status(403).send("complete steps first");
  if (lv[0]!==uid || hu[0]!==uid || lv[1]!==ts || hu[1]!==ts) return res.status(403).send("complete steps first");
  if (lv[2]!==hmacHex(SECRET, `${uid}.${ts}`))   return res.status(403).send("complete steps first");
  if (hu[2]!==hmacHex(SECRET, `${uid}.${ts}.H`)) return res.status(403).send("complete steps first");
...
async function fetchKey(){
  const r = await fetch('/get', { method:'GET' });
...
document.getElementById('btn').onclick = () => fetchKey();
