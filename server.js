...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
        ...
        try {
            ...
            if (data.type === 'connect') {
                ...
                // CRITICAL FIX: Store user in activeUsers Map
                const userData = {
                ...
                console.log(`✅ User stored in activeUsers: ${data.username} (${userId}) - Total users: ${activeUsers.size}`);
                
                // Debug: Show all current users
                console.log(`📋 Current activeUsers:`, Array.from(activeUsers.entries()).map(([id, user]) => `${user.username} (${id})`));
                ...
                if (activeUsers.has(userId)) {
                    ...
                    console.log(`💓 Heartbeat from user ${userId} - lastSeen updated`);
                } else {
                    console.log(`⚠️ Heartbeat from unknown user ${userId} - not in activeUsers`);
                }
                ...
                console.log(`📋 Current activeUsers size: ${activeUsers.size}`);
                console.log(`📋 Current activeUsers:`, Array.from(activeUsers.entries()).map(([id, user]) => `${user.username} (${id})`));
    ...
    ws.on('close', (code, reason) => {
        ...
        if (userId) {
            ...
            // CRITICAL FIX: Remove user from activeUsers when they disconnect
            if (activeUsers.has(userId)) {
                const user = activeUsers.get(userId);
                activeUsers.delete(userId);
                console.log(`✅ User removed from activeUsers: ${user.username} (${userId}) - Total users: ${activeUsers.size}`);
            }
        }
    });
